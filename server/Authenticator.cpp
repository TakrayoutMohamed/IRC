...
void Authenticator::welcomeMsg(const Server &server, const Client &client)
{
	...
	rplWelcome = ":" + client.ip + " 001 "+ client.nickName + " : Welcome to FT_IRC  Network, ";
	rplWelcome += client.nickName +"!"+ client.userName + "@" + client.ip;
	server.sendMsg(rplWelcome, client.fd);
	rplYourhost = ":" + client.ip + " 002 " + client.nickName + " : your host is " + client.serverName + ", runing version 1.1 ";
	server.sendMsg(rplYourhost, client.fd);
	rplCreated = ":" + client.ip + " 003 " + client.nickName + " : this server was created " + getDateTime(server.createDate);
